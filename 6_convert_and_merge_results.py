...
# 权威最终版: 只汇总在 `06_reviewed_results` 文件夹中已审核的结果文件。
...
# 最终报告输出文件夹
FINAL_REPORTS_FOLDER = "07_final_reports" 
if not os.path.exists(FINAL_REPORTS_FOLDER):
    os.makedirs(FINAL_REPORTS_FOLDER)

# 输出格式选择: 'tidy' (一行一码，适合分析) 或 'aggregated' (一行多码，适合浏览和给审核工具使用)
...
def aggregate_codes(coding_results_list):
    ...
    for key, values in aggregated_data.items():
        non_empty_values = [v for v in values if v and v.lower() != 'nan']
        final_aggregated[f'{key}_agg'] = '; '.join(non_empty_values)
    return final_aggregated

def main():
    print(f"--- 开始从文件夹 '{REVIEWED_FOLDER}' 中汇总所有【已审核】的编码结果 ---")
    
    # 【核心修改】只查找已审核的、以 _CORRECTED.jsonl 结尾的结果文件
    result_files_pattern = os.path.join(REVIEWED_FOLDER, "**", "*_CORRECTED.jsonl")
    result_files = glob.glob(result_files_pattern, recursive=True)
    
    if not result_files:
        print(f"错误：在文件夹 '{REVIEWED_FOLDER}' 中未找到任何已审核的结果文件（..._CORRECTED.jsonl）。")
        print("请先运行 5_review_tool.py 对至少一个批次的结果进行审核。")
        return
        
    print(f"找到 {len(result_files)} 个已审核的批次结果文件，准备合并...")
    print('\n'.join(result_files))
    
    df_all = pd.concat([process_jsonl(f) for f in result_files], ignore_index=True)

    if df_all.empty:
        print("所有已审核的结果文件都为空或无效。")
        return

    print(f"共合并了 {len(df_all)} 条记录，正在生成最终报告...")
    
    if OUTPUT_FORMAT == 'tidy':
        FINAL_CSV_OUTPUT_FILE = os.path.join(FINAL_REPORTS_FOLDER, "Final_Coded_Report_CORRECTED_TIDY.csv")
        ...
        FINAL_CSV_OUTPUT_FILE = os.path.join(FINAL_REPORTS_FOLDER, "Final_Coded_Report_CORRECTED_AGGREGATED.csv")
